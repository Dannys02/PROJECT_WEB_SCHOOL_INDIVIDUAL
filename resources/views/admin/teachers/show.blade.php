        ...
        <div class="card-cosmic rounded-lg overflow-hidden">
            ...
            <div class="px-8 py-6 relative">
                <div class="flex items-start justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-3xl font-bold text-white mb-1">{{ $teacher->name ?? 'Tidak ada' }}</h2>
                            <p class="text-blue-100">NIP: {{ $teacher->nip ?? 'Tidak ada' }}</p>
                        </div>
                    </div>
            ...
            <div class="p-8 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <p class="text-white font-semibold text-lg">{{ $teacher->name ?? 'Tidak ada' }}</p>
                        </div>

                        <div>
                            <p class="text-gray-400 text-sm mb-1">Jurusan</p>
                            <p class="text-white font-semibold text-lg">{{ $teacher->major->major_name ?? 'Tidak ada' }}</p>
                        </div>

                        <div>
                            <p class="text-gray-400 text-sm mb-1">Mata Pelajaran</p>
                            <p class="text-white font-semibold text-lg">{{ $teacher->lessons ?? 'Tidak ada' }}</p>
                        </div>

                        <div>
                            <p class="text-gray-400 text-sm mb-1">Nomor Induk Pegawai (NIP)</p>
                            <p class="text-white font-semibold text-lg">{{ $teacher->nip ?? 'Tidak ada' }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="text-white font-semibold text-lg">
                                <span
                                    class="inline-block px-3 py-1 {{ $teacher->gender === 'Laki-laki' ? 'bg-blue-500' : 'bg-pink-500' }} bg-opacity-20 text-purple-300 rounded">
                                    {{ $teacher->gender ?? 'Tidak ada' }}
                                </span>
                            </p>
                        </div>
                ...
                <div class="border-t border-purple-500 border-opacity-30 pt-6">
                    ...
                    <p class="text-white text-base leading-relaxed">{{ $teacher->address ?? 'Tidak ada' }}</p>
                </div>
                ...
                <div class="border-t border-purple-500 border-opacity-30 pt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="stat-card">
                        ...
                        <p class="text-white font-semibold text-sm">
                            {{ $teacher->created_at?->format('d M Y') ?? 'Tidak ada' }}</p>
                    </div>
                    <div class="stat-card">
                        <p class="text-gray-400 text-xs mb-1">Diperbarui</p>
                        <p class="text-white font-semibold text-sm">
                            {{ $teacher->updated_at?->format('d M Y') ?? 'Tidak ada' }}</p>
                    </div>
                    {{-- <div class="stat-card">
                    ...
                    <div class="stat-card">
                        ...
                        <p class="text-white font-semibold text-sm">{{ $teacher->id ?? 'Tidak ada' }}</p>
                    </div>
                </div>
            </div>
        </div>
