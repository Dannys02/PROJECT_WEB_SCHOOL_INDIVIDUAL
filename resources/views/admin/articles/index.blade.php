...
@section('content')
    <div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            ...
            <p class="text-gray-300">Total Artikel: <span class="font-bold text-purple-400">{{ $articles->count() ?? 0 }}</span></p>
        </div>
        <a href="{{ route('admin.articles.create') }}" class="button-cosmic px-6 py-2 rounded-lg text-white font-semibold flex items-center gap-2">
                ...
                <tbody class="text-gray-300">
                    @forelse($articles as $key => $article)
                        <tr>
                            ...
                            <td class="px-6 py-3 font-semibold text-white">{{ $article->judul_artikel ?? 'N/A' }}</td>
                            <td class="px-6 py-3">{{ $article->jurusan?->nama_jurusan ?? 'N/A' }}</td>
                            <td class="px-6 py-3 text-sm">{{ $article->created_at?->format('d M Y') ?? 'N/A' }}</td>
                            <td class="px-6 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('admin.articles.show', $article->id) }}" class="px-3 py-1 bg-blue-500 bg-opacity-20 text-blue-300 rounded hover:bg-opacity-40 transition text-sm">
                                        <i class="fas fa-eye"></i> Lihat
                                    </a>
                                    <a href="{{ route('admin.articles.edit', $article->id) }}" class="px-3 py-1 bg-yellow-500 bg-opacity-20 text-yellow-300 rounded hover:bg-opacity-40 transition text-sm">
