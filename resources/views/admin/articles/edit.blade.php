...
@section('content')
    <div class="max-w-4xl">
        ...
        <div class="card-cosmic rounded-lg p-8">
            <h2 class="text-2xl font-bold text-white mb-6">Edit Artikel: {{ substr($article->judul_artikel ?? 'N/A', 0, 40) }}</h2>
                    ...
                    <div>
                        ...
                        <select id="jurusan_id" name="jurusan_id" class="form-input-cosmic w-full px-4 py-3 rounded-lg">
                            ...
                            @forelse($majors ?? [] as $major)
                                <option value="{{ $major->id }}" @selected(old('jurusan_id', $article->jurusan_id ?? '') == $major->id)>
                                    {{ $major->nama_jurusan }}
                                </option>
                            @empty
                            @endforelse
